...
public class MCBCDataGenMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        ServerPlayConnectionEvents.DISCONNECT.register(
                (handler, server) -> {
                    DataGenerationManager.removePlayer(handler.getPlayer());
                }
        );

        ServerLifecycleEvents.SERVER_STARTED.register(
                server -> {
                    DataGenerationManager.initialize(server.getPlayerManager());
                }
        );
        ServerLifecycleEvents.SERVER_STOPPED.register(
                server -> {
                    DataGenerationManager.clear();
                }
        );
        ServerTickEvents.END_SERVER_TICK.register(
                server -> {
                    DataGenerationManager.tick();
                }
        );
        ...
        ServerPlayNetworking.registerGlobalReceiver(SCREENSHOT_CONFIRMATION_PACKET_ID,
                (server, player, handler, buf, responseSender) -> {
                    DataGenerationManager.handleScreenShotConfirmation(player);
                }
        );
